...
<ul class="cd-tabs-content" style="color: black;">
		...
		<li data-content="applied_leaves" id="applied_leaves">
			<span style="position: absolute; top: 62%;left: 0px;"><b>NL:</b> Normal leave, <b>RL:</b> Retrospective leave</span>
		...
		<li data-content="leave_requests">
			...
			<table class='applied_leaves_table' style="position: relative; top: 10px;">
				...
				$sql = "SELECT * FROM login_info li, emp_info ei, applied_leaves al WHERE (li.username = ei.username AND ei.username = al.username) AND  ( ( (al.approved_by = 'dean_fa' OR approved_by = 'hod_cse' OR approved_by = 'hod_ee' OR approved_by = 'hod_me') AND al.result = 'Pending' AND li.profile_type = 'faculty') );";
				$result = pg_query($conn, $sql);
				while ($row = pg_fetch_assoc($result))
				{	$uid = $row['username'];
		...
		<li data-content="others_leaves">
			...
			<table class='applied_leaves_table' style="position: relative; top: 10px;">
				...
				$sql = "SELECT * FROM login_info li, emp_info ei, applied_leaves al WHERE (li.username = ei.username AND ei.username = al.username) AND  ( (al.approved_by = 'director' AND (li.profile_type <> 'hod_cse' AND li.profile_type <> 'hod_ee' AND li.profile_type <> 'hod_me')) OR ( (al.approved_by = 'dean_fa' OR approved_by = 'hod_cse' OR approved_by = 'hod_ee' OR approved_by = 'hod_me') AND (al.result = 'Declined' OR al.result = 'Approved')) );";
				$result = pg_query($conn, $sql);
				while ($row = pg_fetch_assoc($result))
				{
					$_SESSION['leave_req_username'] = $row['username'];
